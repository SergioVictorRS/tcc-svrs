...
int main() {
    int bsize, m, new_density, now; double density;
    srand(time(NULL));
    printf("Enter nr of zeros to bitmap size: \n");scanf("%d", &bsize);
    ...
    for (uint32_t i = 0; i < bsize; i++) {
        ...
        //if(i%290==0)roaring_bitmap_add(r1, i);
    }
    ...
    start = clock();
    for(int i = 0; i < m; i++){
        rank = roaring_bitmap_rank(r1, (i/m)*bsize);  
    }
    end = clock();
